...
Product direction (#402): work toward a commercial product focused on safety-first, explainable FlaUI locator healing for modern .NET Windows UI tests. Web is a secondary adapter. Intent planning, test generation, and new LLM providers are feature-frozen; existing capabilities receive bug and security fixes. MIT licensing is unchanged. See `docs/external-validation.md` for the pilot protocol; #401 blocks 1.0 until three independent integrations with written feedback and two organic version-drift datasets are verified. Do not count repository-owned samples or synthetic mutations as external validation.
...
- `TestAutomation/Discovery/` — Live UI tree capture via FlaUI.Core/FlaUI.UIA3: `UiTreeWalker`, `ApplicationConnector` (30-second default main-window readiness wait, early-exit detection, unambiguous same-process UIA fallback, and classified startup diagnostics), `DiscoveryOptions` (MaxDepth, MaxElements, Timeout, CancellationToken, IncludeOffscreen, IgnoredControlTypes/ClassNames) and `DiscoveryResult` telemetry. Targets `net48;net8.0-windows` (plus `net10.0-windows` via the `$(NETCoreSdkVersion)` condition) — FlaUI 5.0.0 ships both .NET Framework and modern .NET Windows binaries; UIA3 stays Windows-only regardless of TFM.
...
- `TestAutomation/ScenarioRunner/` — xUnit test suite; the primary test project for the libraries' own behaviour (`TestAutomation/NUnitFixtureTests/` is the one other test project — see below). Contains live UIA scenario tests, live headless-Chromium scenario tests, pure-logic/explainability tests, synthetic benchmarks, and LLM/provider and real-tree locator-ablation harnesses. `LocatorAblationDataset` schema v2 adds nullable multi-locator mutation recipes with per-locator ground truth; `RunMultiLocatorBaseline` measures the current per-locator resolver against one shared mutated tree. `JointLocatorAssignmentEvaluator` is an offline experiment that reconciles only already-accepted top claims under one-candidate ownership. `JointAssignmentGeneralizationDataset` freezes the outcome-independent HandBrake/ShareX sample used by #143; do not retune or filter it after observing results. Both remain outside the production resolver and healing-report API pending #144's explicit product design.
- `TestAutomation/NUnitFixtureTests/` — a `net8.0` NUnit test project (its own `Microsoft.NET.Test.Sdk` project, `IsTestProject`). It exercises the shipped `SelfHealing.Testing.SelfHealingTestBase` pattern end-to-end under NUnit, proving the consumer quickstart works outside the xUnit suite. References only `SelfHealing` and `UiModel`.
- `samples/CalibrationCli/` — .NET 8 CLI calibration tool for evaluating dataset ablation benchmarks and parameter sweeps offline.
- `samples/HeuristicHealingQuickstart/` — .NET 8 console consumer whose committed `.csproj` references the published `AutomationSandbox.SelfHealing` package (never a repo project). Two verify scripts (#336): `verify.ps1` — per-PR, builds a copy with `PackageReference` → `ProjectReference` against current source, no nuget.org (`ci.yml` gates on this); `verify-published.ps1 -Version <v>` — release-time, restores the exact published version from nuget.org into a clean package dir and runs the heal (invoked by `release.yml` after publish).
- `samples/PlaywrightEndToEndQuickstart/` — .NET 8 console consumer demonstrating end-to-end web test generation and live Playwright execution.
...
- Target frameworks:
  ...
  - `Discovery`: `net48;net8.0-windows` (+ `net10.0-windows` conditionally), with `EnableWindowsTargeting` — Windows-only; FlaUI UIA3 needs Windows UIA COM APIs regardless of TFM.
  - `ScenarioRunner`, `WinFormsApp`: `net48` (Windows-only; FlaUI UIA3 needs Windows UIA COM APIs).
  - `WpfApp`: `net8.0-windows` (+ `net10.0-windows` conditionally), with `EnableWindowsTargeting`.
- Key packages: FlaUI.Core/FlaUI.UIA3 5.0.0, System.Text.Json 10.0.12, xunit 2.9.3, Microsoft.NET.Test.Sdk 18.9.0, coverlet.collector 10.0.1, Microsoft.CodeAnalysis.CSharp 5.9.0, xunit.runner.visualstudio 4.0.0.
...
2. `SelfHealingResolver.Resolve` scores every node with `SimilarityScorer` (weighted sum of ControlType 0.20, ParentControlType 0.20, SiblingPosition 0.15, Name/Levenshtein 0.20, Position 0.25 within `PositionToleranceRadius` 300px), prunes below `MinCandidateScore` (0.05), and accepts the top candidate only if its score ≥ `MinimumConfidence` (0.50) **and** its `EvidenceCoverage` (fraction of weight backed by non-null signals) ≥ `MinimumEvidenceWeight` (0.40) **and** the runner-up margin (`best - runnerUp`) ≥ `MinimumCandidateMargin` (0.05, via the shared `UiModel.CandidateMargin` helper). Two **per-component gates** the weighted total cannot override are then applied, both defaulting to disabled and set to a non-zero floor by the `Balanced`/`Conservative` profiles (#370, #375): the **name gate** (`MinimumNameScoreWhenNamed`, 0.30) requires the winning candidate's `NameScore` to clear the floor on its own whenever the stale locator had a name; the **descendant gate** (`MinimumChildSignatureSimilarity`, 0.50) requires the candidate's live direct-child `ControlType` multiset to match the one recorded in `UiElementInfo.ChildControlTypeSignature` (a `UiElementSnapshot.Capture` field, null on pre-#375 snapshots and empty on leaves) whenever the stale locator was a container. The evidence gate and both per-component gates apply to LLM picks too; the margin gate does not.
...
- The seven library project files enable XML documentation; `Directory.Build.targets` enables analyzers and documentation warning gates after `IsPackable` is known. Missing public XML comments are build errors. Keep the source API inventory current with `dotnet run --project eng/PublicApiAudit -- . docs/public-api.json`; `docs/public-api-audit.md` records deliberate mutability decisions. `ScoreComponents`, `PlaywrightLocatorSuggestion`, `IntentElementCandidate`, and `IntentDesktopElementCandidate` use get-only properties and named constructor arguments. Candidate references are not deep-frozen, but their locator-suggestion collections are defensively copied. Package validation checks XML documentation beside every packaged DLL.
...
- `.github/workflows/ci.yml` — on push/PR to `main` and manual dispatch, runs across a matrix (`windows-latest` for `net48` full suite including FlaUI live tests; `ubuntu-latest` for `net8.0` cross-platform core and web suite): sets up .NET 8 and 10 SDKs, restores, builds Debug, audits packages (see below), installs the Playwright Chromium browser (`playwright.ps1 install chromium`), runs ScenarioRunner tests with a TRX logger and collects coverage (Linux via coverlet `XPlat Code Coverage`; Windows `net48` via pinned `dotnet-coverage` static instrumentation filtered to repo assemblies by `.github/config/windows-coverage.runsettings` (#289), because coverlet 8.0+ ships no .NET Framework-loadable collector), renders separate `Windows net48` / `Linux net8.0` overall and per-assembly coverage tables into each job's `$GITHUB_STEP_SUMMARY`, and keeps uploading test results and Cobertura files as artifacts. A separate required Linux job (`Sample Compiles Against Source`) runs `samples/HeuristicHealingQuickstart/verify.ps1`, which builds and runs the sample against the **current engine source** (its `PackageReference` swapped for a `ProjectReference`) — it does not touch nuget.org, so a version bump never blocks a PR (#336). The "the published package restores and works for a consumer" check moved to `release.yml` as a post-publish step. Coverage is visibility-only: never combine the legs, add a badge/threshold, or fail a build on a coverage delta. `ANTHROPIC_API_KEY`/`GEMINI_API_KEY` repo secrets are optional; the LLM comparison test self-skips without them. The package security audit is the deliberate exception to "coverage is visibility-only": High/Critical vulnerability advisories are a merge gate, not just a report (see Security considerations below).
...
- `.github/workflows/release.yml` — manual `workflow_dispatch` with a `dry_run` input; packs the same seven libraries, generates one CycloneDX JSON SBOM per package with a pinned CycloneDX tool version (uploaded as the `release-sboms` artifact and attached to the GitHub Release next to the packages, #274), uploads artifacts (`if: always()`), and creates the GitHub release. Release notes are built by reading `docs/release-notes/<tag>.md` (or `<version>.md`), performing `__TAG__` and `__NUGET_PUBLISH_LINE__` placeholder substitutions, and passing the resulting file to `gh release create --notes-file`. When `publish_to_nuget` is set, two post-publish steps run (#336): `verify-published.ps1 -Version <just-published>` proves an external consumer can restore the exact version from nuget.org (with an indexing-lag retry loop), then a bump step rewrites `Directory.Build.props` `<Version>` and the `HeuristicHealingQuickstart` pin to that version and opens a `release/ssot-bump-VERSION` PR. Needs `pull-requests: write`.

  **Release sequence (single action).** Write `docs/release-notes/vVERSION.md`, then dispatch `release.yml` with the `version` input set and `publish_to_nuget=true`. The run packs, publishes, verifies the live package, and opens the SSoT-bump PR. Merge that PR once its CI is green — nothing else is manual. Do **not** bump `<Version>` by hand first; that is what caused the chicken-and-egg the old `nuget-consumer` per-PR job hit on every release.
- `.github/workflows/llm-smoke.yml` — live smoke test against GitHub Models or any OpenAI-compatible endpoint on demand (`workflow_dispatch`). The nightly schedule is deliberately commented out: GitHub Models first answered `HTTP 410 github_models_retirement_brownout` and its inference API was then fully retired on July 30, 2026; a nightly nobody can fix trains everyone to ignore red (#44). Endpoint and model come from repo variables, so switching backends is two variables and a secret, no code change.
- `.github/workflows/nightly-consensus.yml` — nightly (02:00 UTC) and manual multi-provider consensus evaluation on Linux/`net8.0` (#47). Its first live step is a **gating** pool assertion on `Desktop_AmbiguousSiblingTabs` (#84, redesigned 2-of-N in #388): the whole configured provider pool is exercised, and at least `MinimumConsensusVotes` (2) providers must return shortlist-valid votes agreeing on the ground-truth candidate and appear in `AgreedProviders`, with the outcome written to `$GITHUB_STEP_SUMMARY`. A provider down with HTTP 429/timeout contributes no vote and does not fail the gate by itself; fewer than 2 usable votes is a hard failure because quorum is then not demonstrable. No credentials means a clean opt-in skip; fewer than two configured providers is a hard failure proving quorum is real. The broader `CONSENSUS_EVALUATION=1` run follows as non-gating data collection, includes optional Cloudflare Workers AI when its token/account/model triplet is configured, writes the full JSON artifact and Markdown table, and may record provider rate limits/failures without failing on its aggregate metrics. These flags are set only by this workflow so `ci.yml` does not consume live quota on each PR. `GITHUB_TOKEN` is deliberately **not** exported: `OpenAiHealingProvider` could otherwise send it to the wrong endpoint. The live gate is nightly-only, not a release gate, because third-party availability must not block a manual package release.
- `.github/workflows/ablation-consensus.yml` — runs real-tree multi-signal locator ablation sweeps across perturbation tiers.
- `.github/workflows/provider-diagnostics.yml` — diagnostic connectivity, response parsing, and schema validation workflow for configured LLM provider backends.
- `.github/workflows/windows-app-survey.yml` — FlaUI desktop survey workflow executing discovery and telemetry extraction across Windows desktop applications.
- `.github/workflows/open-source-app-survey.yml` — surveys open-source Windows applications for locator stability benchmarking.
- `.github/workflows/docs.yml` — publishes `docs/` to GitHub Pages.
- `.github/workflows/dora-metrics.yml` — weekly (Monday 06:00 UTC) and manual DORA/engine-reliability report (#323): runs `scripts/calculate_dora.ps1` (pwsh — tooling stays C#/PowerShell-only, no Python), writes the report to the job summary and uploads it as the `dora-report` artifact. Visibility-only reporting; it gates nothing. The script exits non-zero on a GitHub API fetch failure rather than publishing a report built on empty data.
