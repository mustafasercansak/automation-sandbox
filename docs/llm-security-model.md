...
| One provider returns an arbitrary high-confidence answer | At least `MinimumConsensusVotes` independent provider votes must name the same in-shortlist candidate; self-reported confidence does not gate acceptance. The per-component name and descendant/child-signature gates apply to the agreed candidate identically to a heuristic winner (#416), so a name- or content-mismatched pick is declined even after agreement. | Correctness or security. Providers can share the same bad assumption, and agreement is explicitly not evidence that an element still exists. |
...
| DOM/UI metnindeki prompt injection (ör. "önceki talimatları yok say") locator seçimini ele geçirmeye çalışır | Yapısal XML sınır etiketleri (`<target_element>`, `<candidate_shortlist>`, `<test_intent>`), modelin eleman içeriğini yalnızca pasif veri olarak değerlendirmesini emreden açık sistem güvenlik direktifi, JSON öznitelik kaçışı, sentetik candidateId kapalı kümesi, hallucination guard ve çoklu sağlayıcı uzlaşma quorum'u. `LlmIntentPlanningPrompt.Build`, gönderdiği güvenilmeyen `Goal`/`TargetUrl`/`TestData` metnine de aynı sınır etiketi + direktif desenini uygular (`<test_goal>`, `<target_url>`, `<test_data>`); gömülü metnin sınırı erken kapatamaması için etiket kaçışı da dahildir. | Liste içindeki yanlış bir adayı seçmesi için tüm uzlaşma sağlayıcılarını başarıyla aldatan incelikli bir in-context jailbreak. |
...
| Tek sağlayıcı keyfî ve yüksek-confidence bir cevap verir | En az `MinimumConsensusVotes` bağımsız sağlayıcı oyu aynı liste içi adayı göstermelidir; modelin confidence değeri kabulü belirlemez. İsim ve descendant/child-signature per-component kapıları, anlaşılan adaya da sezgisel kazanana uygulandığı gibi uygulanır (#416); bu yüzden isim veya içerik uyuşmazlığı olan bir seçim, uzlaşmaya rağmen reddedilir. | Doğruluk veya güvenlik. Sağlayıcılar aynı hatalı varsayımı paylaşabilir; uzlaşma elemanın hâlâ var olduğunu kanıtlamaz. |
