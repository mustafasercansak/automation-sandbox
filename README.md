...
| **Long-Lived Web Sessions** | ✅ Implemented | `PlaywrightWebSession` (`AutomationSandbox.PlaywrightLiveExploration`) reuses one browser page across multiple `NavigateAsync`/`CaptureAsync` calls instead of `PlaywrightLiveExplorer`'s per-call page, and records console messages, HTTP responses, request failures, and uncaught page errors observed for the life of the session — the foundation for authenticate-once, drive-many-actions automation. |
...
| **Long-Lived Web Sessions** | Multi-navigation page reuse, and console/network-response/request-failure/page-error capture, via `PlaywrightWebSession` against real headless Chromium | [PlaywrightWebSessionTests](TestAutomation/ScenarioRunner/PlaywrightWebSessionTests.cs) |
