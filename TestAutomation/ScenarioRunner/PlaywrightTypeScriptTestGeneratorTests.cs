...
namespace ScenarioRunner
{
    public class PlaywrightTypeScriptTestGeneratorTests
    {
        ...
        {
            // A Wait step reuses AssertionKind/ExpectedValue (not just Assert steps): expect(...).toHaveText
            // already polls until the condition holds or the timeout elapses, so it doubles as a text-based
            // wait - the shape needed for something like a language switch, where a plain visibility wait on
            // an already-visible element would resolve immediately and prove nothing.
            var scenario = new IntentScenario
            {
            ...
            Assert.Contains("await expect(page.getByTestId('greeting')).toHaveText('Merhaba', { timeout: 3000 });", code);
        }
        ...
        public void Generate_EmitsWaitForTextContains_WhenWaitStepHasTextContainsAssertionKind()
        {
            ...
            Assert.Contains("await expect(page.getByTestId('banner')).toContainText('50% off', { timeout: 2000 });", code);
        }
        ...
        public void Generate_EmitsFallbackVisibilityAssertion_WithoutTodoComments_WhenAssertGenerationModeIsFallback()
        {
            ...
            var code = new PlaywrightTypeScriptTestGenerator(new PlaywrightTypeScriptTestGenerationOptions { AssertGenerationMode = AssertGenerationMode.Fallback }).Generate(scenario, recordings);

            Assert.DoesNotContain("// TODO:", code);
            Assert.DoesNotContain("test.skip", code);
            Assert.Contains("await expect(page.getByTestId('outcome-box')).toBeVisible();", code);
        }
