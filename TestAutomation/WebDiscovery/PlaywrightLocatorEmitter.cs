    ...
    public static class PlaywrightLocatorEmitter
    {
        public static IReadOnlyList<PlaywrightLocatorSuggestion> Suggest(WebElementInfo element)
        {
            ...
            if (!string.IsNullOrWhiteSpace(element.TestId))
            {
                ...
                {
                    Strategy = "TestId",
                    Expression = $"page.GetByTestId(\"{Escape(element.TestId)}\")",
                    Confidence = 0.98,
            ...
            if (!string.IsNullOrWhiteSpace(element.Role) && !string.IsNullOrWhiteSpace(element.AccessibleName))
            {
                ...
                {
                    Strategy = "Role",
                    Expression = $"page.GetByRole(AriaRole.{ToAriaRole(element.Role)}, new() {{ Name = \"{Escape(element.AccessibleName)}\" }})",
                    Confidence = 0.90,
            ...
            if (!string.IsNullOrWhiteSpace(element.Id))
            {
                ...
                {
                    Strategy = "Id",
                    Expression = $"page.Locator(\"#{EscapeCssId(element.Id)}\")",
                    Confidence = 0.78,
            ...
            if (!string.IsNullOrWhiteSpace(element.NameAttribute))
            {
                suggestions.Add(new PlaywrightLocatorSuggestion
                {
                    ...
                    Expression = $"page.Locator(\"[name='{EscapeCssString(element.NameAttribute)}']\")",
                    Confidence = 0.70,
            ...
            if (!string.IsNullOrWhiteSpace(element.CssSelector))
            {
                ...
                {
                    Strategy = "Css",
                    Expression = $"page.Locator(\"{Escape(element.CssSelector)}\")",
                    Confidence = 0.55,
